# Jotai State Tree SSR Starter Template

This is a comprehensive, production-ready starter template for building Server-Side Rendered (SSR) React applications using **Jotai State Tree** (`jotai-state-tree`), React, TypeScript, and Tailwind CSS. It is designed to mirror the structure of frameworks like TanStack Start, Next.js, and RedwoodJS, offering a fully modular structure, server/client hydration, remote Server Actions with patch-sync, built-in URL routing, state persistence, undo/redo managers, and a dark/light mode dashboard.
...
```bash
npx degit bmartel/jotai-state-tree/examples/project-starter-ssr my-new-app
cd my-new-app
```
...
│       └── testing-guide/
├── server.js              # Node.js/Express dev & prod server with SSR & actions handler
├── src/
│   ├── entry-client.tsx   # Client-side mounting/hydration entry
│   ├── entry-server.tsx   # Server-side HTML render string entry
