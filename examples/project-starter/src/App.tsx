...
import { observer, RouteView, RouterContext } from 'jotai-state-tree/react';
import {
  onPatch,
  onSnapshot,
  applySnapshot,
...
export function useAppStore() {
  ...
  if (!store) {
    throw new Error(
      '[jotai-state-tree] useAppStore must be used within StoreContext.Provider'
    );
  }
  return store;
}
...
export const App = observer(function App() {
  ...
  // 1. Initialize store, router, and undoManager once
  const { store, router, undoManager } = useMemo(() => {
    const s = createAppStore();
    const r = configureRouter(s);
    const u = createUndoManager(s.tasks);
